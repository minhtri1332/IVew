...
export const requestGetDepartment = async () => {
    const {data} = await Fetch.get<{data: any}>('https://go.iview.vn/api/v1/department/show-department', {
        params: {
            page:1,
            limit:100
        }
    ...
    batch(() => {
        ...
        setDepartmentQueries({ all: data.data.listDepartment.map((item:RawDepartment) => item.id) });
    });
};
